...
var DataImport=require("./data-import.js"),Grid=require("./grid.js"),HUD=require("./hud.js"),Layer=require("./layer.js"),Legend=require("./legend.js"),Utils=require("./utils.js");try{new Uint8ClampedArray}catch(e){window.Uint8ClampedArray=Uint8Array}var defaultColorScale=d3.scale.linear().domain([0,255]).range(["#a50026","#d73027","#f46d43","#fdae61","#fee08b","#d9ef8b","#a6d96a","#66bd63","#1a9850","#006837"]),GridMap=function(t,e){var i=this;this.container=d3.select(t);var o=this.container.node().getBoundingClientRect();this.width=0|o.width,this.height=0|o.height,this.layers=[],this.options=e||{},this.seaColor=this.options.seaColor||"rgba(21,98,180,.8)",this.graticuleColor=this.options.graticuleColor||"rgba(255,255,255,.3)",this.graticuleWidth=this.options.graticuleWidth||1;var n=-this.options.latitude||0,r=-this.options.longitude||0,s=this.options.scale||150;i.area=1,this.dispatch=d3.geo.GridMap.dispatch,this.projection=this.options.projection||d3.geo.aitoff(),this.projection.translate([this.width/2,this.height/2]).clipExtent([[0,0],[i.width,i.height]]).scale(s).rotate([r,n]),this.canvas=this.container.append("canvas").style("position","absolute").style("top","0px").style("left","0px"),this.canvas.data([0]),this.context=this.canvas.node().getContext("2d");var a=d3.geo.transform({point:function(t,e,o){o>=i.area&&this.stream.point(t,e)}});this.simplifyingPath=d3.geo.path().projection({stream:function(t){return a.stream(i.projection.stream(t))}}),this.path=d3.geo.path().projection(this.projection);var d=new HUD(this,e.hud);this.colorScale=this.options.colorScale||defaultColorScale,this.options.zoomLevels||(this.options.zoomLevels=[1,2,4,8]),this.options.legend&&(this.options.context=d.context,this.options.colorScale=this.colorScale,this.legend=new Legend(this.options),this.legend.draw()),this.init=function(){this.initEvents(),this.resize()},this.getGrid=function(){for(var t=[],e=0;e<this.layers.length;e++)this.layers[e].grid&&this.layers[e].visible&&t.push(this.layers[e].grid);return 1===t.length?t[0]:t.length>0?t:void 0},this.onMouseMove=function(){if(i.options.onCellHover||i.options.hud){var t=i.projection.invert(d3.mouse(this));if(t){var o=null,n=null,r=i.getGrid();r&&t[0]&&t[1]&&t[0]>-180&&t[0]<180&&t[1]>-90&&t[1]<90&&(o=r.coordinatesToCellId(t),n=r.getCell(o),n&&i.options.onCellHover&&i.options.onCellHover(n,o)),e.hud&&o&&d.update(o,t,n),i.legend&&(i.legend.draw(),i.legend.highlight(n))}}},this.initEvents=function(){var t=d3.behavior.drag().on("dragstart",function(){}).on("drag",function(){r+=100*d3.event.dx/s,n-=100*d3.event.dy/s,i.projection.rotate([r,n]),i.drawAnimation()}).on("dragend",function(){i.draw()});if(!i.options.disableMouseZoom){var e=d3.behavior.zoom().on("zoomstart",function(){}).on("zoomend",function(){i.draw()}).on("zoom",function(t){s=d3.event.scale,i.area=2e4/s/s,i.projection.scale(s),i.drawAnimation()}).scale(s).scaleExtent([0,4e3]);this.container.call(e)}this.container.call(t),this.container.on("mousemove",i.onMouseMove),d3.select(window).on("resize",d3.geo.GridMap.dispatch.resize),d3.geo.GridMap.dispatch.on("resize."+i.container.attr("id"),function(){i.resize()})},this.drawWorld=function(){this.context.clearRect(0,0,this.width,this.height),this.context.beginPath(),this.path.context(this.context)({type:"Sphere"}),this.context.fillStyle=this.seaColor,this.context.fill()},this.drawLayers=function(t){for(var e=0;e<i.layers.length;e++){var o=i.layers[e],n=!t||o.options.renderOnAnimate;n?o.draw():o.clear()}},this._draw=function(){i.dispatch.drawStart(),i.drawWorld(),i.drawLayers(),d.draw(),i.dispatch.drawEnd()},this.drawAnimation=function(){var t=!0;i.drawWorld(),i.drawLayers(t),d.draw()};var h=function(t,e){var i=-1;return function(){i>-1&&window.clearTimeout(i),i=window.setTimeout(t,e)}};i.draw=h(i._draw,500),this._resize=function(){var t=i.container.node().getBoundingClientRect();i.width=0|t.width,i.height=0|t.height,i.canvas.attr("width",i.width),i.canvas.attr("height",i.height);for(var e=0;e<i.layers.length;e++)i.layers[e].resize(i.width,i.height);i.projection.translate([i.width/2,i.height/2]).clipExtent([[0,0],[i.width,i.height]]),d.resize(i.width,i.height),i.draw()},this.resize=h(i._resize,200),this.panToCentroid=function(t){var e=d3.geo.centroid(t).map(Math.round),i=this.projection.rotate().map(Math.round);i[0]=-e[0],this.projection.rotate(i)},this.addLayer=function(t,e){var o=new Layer(i,e);if(t.BYTES_PER_ELEMENT){var n=e&&e.colorScale||i.colorScale;e.colorScaleDiscrete&&n.range(n.range().map(Utils.colorStringToUint32)),o.grid=DataImport.arrayToGrid(t,e.gridSize,n)}else{if("Topology"===t.type){var r=e&&e.topojsonObject||t.objects[Object.keys(t.objects)[0]];t=topojson.feature(topojson.presimplify(t),r),o.simplified=!0}o.json=t}return i.layers.push(o),this.container.selectAll("canvas").sort(),e&&(e.renderOnAdd||void 0==e.renderOnAdd)&&i.draw(),o},this.removeLayer=function(t){if("number"==typeof t)t=i.layers.splice(t,1)[0];else for(var e=0;e<i.layers.length;e++)i.layers[e]===t&&i.layers.splice(e,1);return t.remove(),t},this.zoomTo=function(t){i.area=2e4/t/t,i.projection.scale(t),i.draw()},this.zoomIn=function(){i.options.zoomLevels.sort(function(t,e){return t-e});for(var t=i.projection.scale(),e=0;e<i.options.zoomLevels.length;e++)if(150*i.options.zoomLevels[e]>t)return void i.zoomTo(150*i.options.zoomLevels[e])},this.zoomOut=function(){i.options.zoomLevels.sort(function(t,e){return t-e});for(var t=i.projection.scale(),e=i.options.zoomLevels.length-1;e>=0;e--)if(150*i.options.zoomLevels[e]<t)return void i.zoomTo(150*i.options.zoomLevels[e])},this.init()};window.d3.geo.GridMap=GridMap,window.d3.geo.GridMap.dispatch=d3.geo.GridMap.dispatch||d3.dispatch("drawStart","drawEnd","resize");
