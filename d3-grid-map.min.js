...
var Grid=require("./grid.js"),HUD=require("./hud.js"),Layer=require("./layer.js"),Legend=require("./legend.js"),DataImport=require("./data-import.js");try{new Uint8ClampedArray}catch(e){window.Uint8ClampedArray=Uint8Array}var defaultColorScale=d3.scale.quantize().domain([0,255]).range(["#a50026","#d73027","#f46d43","#fdae61","#fee08b","#d9ef8b","#a6d96a","#66bd63","#1a9850","#006837"]),GridMap=function(t,o){var e=this,i=0,r=0;this.container=d3.select(t);var n=d3.geo.graticule()(),a=this.container.node().getBoundingClientRect();this.width=0|a.width,this.height=0|a.height,this.layers=[],this.options=o||{},this.seaColor=this.options.seaColor||"rgba(21,98,180,.8)",this.graticuleColor=this.options.graticuleColor||"rgba(255,255,255,.3)",e.area=1;var s=d3.geo.transform({point:function(t,o,i){i>=e.area&&this.stream.point(t,o)}});this.dispatch=d3.geo.GridMap.dispatch,this.projection=this.options.projection||d3.geo.aitoff(),this.projection.translate([this.width/2,this.height/2]).clipExtent([[0,0],[e.width,e.height]]).precision(.1),this.canvas=this.container.append("canvas").style("position","absolute").style("top","0px").style("left","0px"),this.context=this.canvas.node().getContext("2d");var d=null;o.hud&&(d=new HUD(this)),this.colorScale=this.options.colorScale||defaultColorScale,this.options.zoomLevels||(this.options.zoomLevels=[1,2,4,8]),this.options.legend&&d&&(this.options.context=d.context,this.options.colorScale=this.colorScale,this.legend=new Legend(this.options),this.legend.draw()),this.simplifyingPath=d3.geo.path().projection({stream:function(t){return s.stream(e.projection.stream(t))}}).context(this.context),this.path=d3.geo.path().projection(this.projection).context(this.context),this.init=function(){this.initEvents(),this.resize()},this.getGrid=function(){for(var t=0;t<this.layers.length;t++)if(this.layers[t].grid)return this.layers[t].grid},this.onMouseMove=function(){if(e.options.onCellHover||e.options.hud){var t=e.projection.invert(d3.mouse(this));if(t){var o=null,i=null,r=e.getGrid();r&&t[0]&&t[1]&&t[0]>-180&&t[0]<180&&t[1]>-90&&t[1]<90&&(o=r.coordinatesToCellId(t),i=r.getCell(o),i&&e.options.onCellHover&&e.options.onCellHover(i,o));var n=i/255;d&&o&&d.update(o,t,n),e.legend&&(e.legend.draw(),e.legend.highlight(n))}}},this.initEvents=function(){var t=150,o=d3.behavior.drag().on("dragstart",function(){}).on("drag",function(){r+=100*d3.event.dx/t,i-=100*d3.event.dy/t,e.projection.rotate([r,i]),e.drawAnimation()}).on("dragend",function(){e.draw()});if(!e.options.disableMouseZoom){var n=d3.behavior.zoom().on("zoomstart",function(){}).on("zoomend",function(){e.draw()}).on("zoom",function(o){t=d3.event.scale,e.area=2e4/t/t,e.projection.scale(t),e.drawAnimation()}).scale(t).scaleExtent([0,4e3]);this.container.call(n)}this.container.call(o),this.container.on("mousemove",e.onMouseMove),d3.select(window).on("resize",d3.geo.GridMap.dispatch.resize),d3.geo.GridMap.dispatch.on("resize."+e.container.attr("id"),function(){e.resize()})},this.drawWorld=function(){this.context.clearRect(0,0,this.width,this.height),this.context.beginPath(),this.path({type:"Sphere"}),this.context.fillStyle=this.seaColor,this.context.fill()},this.drawGeoJSONLayer=function(t){e.context.beginPath(),t.simplified?e.simplifyingPath(t.json):e.path(t.json),e.context.strokeStyle=t.options.strokeColor,e.context.lineWidth=.5,e.context.stroke(),e.context.fillStyle=t.options.fillColor,e.context.fill()},this.screenCoordinatesToGridIndex=function(t,o,e){var i=o.invert(t);if(i){var r=i[0],n=i[1];if(180>=r&&r>=-180&&90>=n&&n>=-90){var a=~~(~~((90-n)/180*e.rows)*e.cols+(180+r)/360*e.cols+1);return a}}},this.renderGridToCanvas=function(t,o){for(var i=this.context.getImageData(0,0,this.width,this.height),r=i.data,n=0;n<o.length;n++){var a=4*n,s=4*o[n];0!==t.data[s+3]&&(r[a]=t.data[s],r[a++]=t.data[s++],r[a++]=t.data[s++],r[a++]=t.data[s++])}e.context.putImageData(i,0,0)},this.drawGrid=function(t){for(var o=[],i=0;i<this.height;i++)for(var r=0;r<this.width;r++){var n=r+this.width*i,a=this.screenCoordinatesToGridIndex([r,i],e.projection,t);o[n]=a}this.renderGridToCanvas(t,o)},this.drawGraticule=function(){this.context.beginPath(),this.path(n),this.context.closePath(),this.context.lineWidth=1,this.context.strokeStyle=this.graticuleColor,this.context.stroke()},this.drawLayers=function(t){for(var o=0;o<e.layers.length;o++){var i=e.layers[o],r=!t||i.options.renderOnAnimate;r&&(i.grid?e.drawGrid(i.grid):i.json&&("Topology"===i.json.type?e.drawTopoJSONLayer(i):e.drawGeoJSONLayer(i)))}},this._draw=function(){e.dispatch.drawStart(),e.drawWorld(),e.drawLayers(),e.drawGraticule(),e.dispatch.drawEnd()},this.drawAnimation=function(){var t=!0;e.drawWorld(),e.drawLayers(t),e.drawGraticule()};var h=function(t,o){var e=-1;return function(){e>-1&&window.clearTimeout(e),e=window.setTimeout(t,o)}};e.draw=h(e._draw,500),this._resize=function(){console.log("resizing ",e);var t=e.container.node().getBoundingClientRect();e.width=0|t.width,e.height=0|t.height,e.canvas.attr("width",e.width),e.canvas.attr("height",e.height),d&&d.resize(e.width,e.height),e.projection.translate([e.width/2,e.height/2]).clipExtent([[0,0],[e.width,e.height]]),e.draw()},this.resize=h(e._resize,200),this.panToCentroid=function(t){var o=d3.geo.centroid(t).map(Math.round),e=this.projection.rotate().map(Math.round);e[0]=-o[0],this.projection.rotate(e)},this.addLayer=function(t,o){var i=new Layer(o);if(t.constructor===ArrayBuffer){var r=DataImport.arrayBufferToGrid(t,o.gridSize,e.colorScale);i.grid=r}else if(t.constructor===Uint8Array||t.constructor===Uint8ClampedArray){var r=new Grid(t,o.gridSize);i.grid=r}else{if("Topology"===t.type){var n=o&&o.topojsonObject||t.objects[Object.keys(t.objects)[0]];t=topojson.feature(topojson.presimplify(t),n),i.simplified=!0}i.json=t}return e.layers.push(i),e.layers.sort(function(t,o){return t.options.zIndex-o.options.zIndex}),e.draw(),i},this.removeLayer=function(t){if("number"==typeof t)e.layers.splice(t,1);else for(var o=0;o<e.layers.length;o++)e.layers[o]===t&&e.layers.splice(o,1)},this.zoomTo=function(t){e.area=2e4/t/t,e.projection.scale(t),e.draw()},this.zoomIn=function(){e.options.zoomLevels.sort(function(t,o){return t-o});for(var t=e.projection.scale(),o=0;o<e.options.zoomLevels.length;o++)if(150*e.options.zoomLevels[o]>t)return void e.zoomTo(150*e.options.zoomLevels[o])},this.zoomOut=function(){e.options.zoomLevels.sort(function(t,o){return t-o});for(var t=e.projection.scale(),o=e.options.zoomLevels.length-1;o>=0;o--)if(150*e.options.zoomLevels[o]<t)return void e.zoomTo(150*e.options.zoomLevels[o])},this.init()};window.d3.geo.GridMap=GridMap,window.d3.geo.GridMap.dispatch=d3.geo.GridMap.dispatch||d3.dispatch("drawStart","drawEnd","resize");
